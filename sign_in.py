...
class Signin():
    ...
    def screen (self):
        ...
        while True:
            ...
            #WORDHINT:
            #---------------------
            wordhint_surface= self.myPygame.text.render("LOGIN TO PLAY", True, self.color.WHITE)
            ...
            signup_surface= self.myPygame.text.render("Don't have Account ?  Press SPACE to signup", True, self.color.RED)
    ...
    def signin(self):
        ...
        login = db.Login_db(username, password)
        if login.get_login_state() == 'OK':
            spellme.Spellme(username).start()
        else:
            self.error_msg = login.get_login_state()
